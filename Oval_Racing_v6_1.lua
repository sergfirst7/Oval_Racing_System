...
local function applyRealPenalty(id, seconds)
    -- Мы не можем использовать нативные команды типа /add_time без плагина и прав админа.
    -- Поэтому отправляем команду твоему серверному плагину (!penalty)
    ac.sendChatMessage("!penalty #" .. id .. " " .. seconds)
    ac.sendChatMessage("Penalty applied: " .. seconds .. "s")
    ...
    local ok, err = pcall(function()
        ...
        pcall(function()
            ...
            if sess then
                ...
                debugSessionType = "No Sess Object"
                isRace = (s.sessionType == 2 or s.sessionType == ac.SessionType.Race)
            end
        end)
            ...
            if sessionStartTimer > 5 then
                ...
                if distToTarget < -1 then
                    if warningOvertakeTimer > 0 then
                        ...
                        if warningOvertakeTimer <= 0 then
                            applyRealPenalty(ac.getCarID(0), 10)
