...
local function applyRealPenalty(id, seconds)
    -- У игрока нет плагина на сервере. Реализуем локальный Stop-And-Go штраф.
    if id == ac.getCarID(0) or id == 0 then
        activePenaltySeconds = activePenaltySeconds + seconds
        ac.sendChatMessage("System: " .. seconds .. "s STOP-AND-GO Penalty Applied!")
    end
end
...
function script.update(dt)
    ...
    local ok, err = pcall(function()
        ...
        pcall(function()
            ...
            if sess then
                ...
                elseif sess.type == ac.SessionType.Race or sess.type == 2 or sname:find("race") then
                    ...
                    isRace = false -- Default to false if we don't know it's a race
                end
            else
                -- Fallback for Emperor Server on older CSP where ac.getSession fails
                -- В овалах гонка всегда на круги (maxLaps > 0). Квала/Практика обычно на время (maxLaps = 0)
        ...
        if isOvalSystemActive and isRace then
            if not raceStarted then
                local anyMoving = false
                for i = 0, s.carsCount - 1 do
                    local c = ac.getCar(i)
                    if c and c.speedKmh > 50 then
                        anyMoving = true
                        break
                    end
                end
                if anyMoving then
                    raceStartConfirmTimer = raceStartConfirmTimer + dt
                    if raceStartConfirmTimer > 1.0 then
                        raceStarted = true
                    end
                else
                    raceStartConfirmTimer = 0
                end
            else
        ...
        -- Проверка замедления
        if currentFlag == STATE.GREEN and yellowCooldown <= 0 and isOvalSystemActive and isRace and raceStarted and timeSinceRaceStart > 5 then
            if not me.isInPitlane and me.speedKmh < 40 then
                ...
                if slowTimers[0] > 2.0 then 
                    ...
                    cautionStartSpline = me.splinePosition
                    initTarget()
                    yellowCooldown = 30 
                end
            else
        ...
        if currentFlag ~= STATE.GREEN then
            ...
            if sessionStartTimer > 5 then
                -- Overtake
                if distToTarget < -1 then
                    if warningOvertakeTimer > 0 then
                        warningOvertakeTimer = warningOvertakeTimer - dt
                        if warningOvertakeTimer <= 0 then
                            applyRealPenalty(0, 10)
                            warningOvertakeTimer = -10 -- cooloff
                        end
                    elseif warningOvertakeTimer < 0 then
                        warningOvertakeTimer = math.min(0, warningOvertakeTimer + dt)
                    end
                else
                    warningOvertakeTimer = 5.0
                end

                -- Lag
                if distToTarget > 50 then
                    if warningLagTimer > 0 then
                        warningLagTimer = warningLagTimer - dt
                        if warningLagTimer <= 0 then
                            applyRealPenalty(0, 10)
                            warningLagTimer = -10 -- cooloff
                        end
                    elseif warningLagTimer < 0 then
                        warningLagTimer = math.min(0, warningLagTimer + dt)
                    end
                else
                    warningLagTimer = 15.0
                end
            end

            local lapsPassed = (me.lapCount + me.splinePosition) - (cautionStartLap + cautionStartSpline)
            if lapsPassed >= 2.0 then
                currentFlag = STATE.GREEN
                targetCarID = -1
                restartTimer = 2.0
        ...
        -- Обработка локального штрафа
        if activePenaltySeconds > 0 then
            if me.speedKmh < 1.0 then
                activePenaltySeconds = math.max(0, activePenaltySeconds - dt)
            end
        end
    end)
...
ac.onChatMessage(function(msg, senderID)
    ...
    if string.find(text, "!yellow") then 
        ...
        cautionStartSpline = ac.getCar(0).splinePosition
        initTarget()
        yellowCooldown = 30
    elseif string.find(text, "!green") then 
        ...
        restartTimer = 2.0
        yellowCooldown = 10
    elseif string.find(text, "!oval off") then
        isOvalSystemActive = false
        ac.sendChatMessage("System: Oval Racing Disabled for Qualy")
    elseif string.find(text, "!oval on") then
        isOvalSystemActive = true
        ac.sendChatMessage("System: Oval Racing Enabled for Race")
    end
end)
...
function script.drawUI()
    ...
    pcall(function()
        ...
        if showBlock then
            ...
            elseif currentFlag == STATE.ONE_TO_GREEN then title = "SC IN THIS LAP / PREPARE" end
            ...
            if currentFlag ~= STATE.GREEN then
                ...
                ui.textColored(dText, (dist < -1 or dist > 50) and rgbm(1, 0, 0, 1) or rgbm(0, 0, 0, 1))
                ui.popFont()

                if warningOvertakeTimer > 0 and warningOvertakeTimer < 5.0 then
                    local warn1 = "RETURN TO POSITION! " .. string.format("%.1f", warningOvertakeTimer) .. "s"
                    ui.setCursor(vec2(centerX - ui.measureText(warn1).x/2, boxY + 120))
                    ui.textColored(warn1, rgbm(1, 0, 0, 1))
                end
                
                if warningLagTimer > 0 and warningLagTimer < 15.0 then
                    local warn2 = "CATCH UP TO SC! " .. string.format("%.1f", warningLagTimer) .. "s"
                    ui.setCursor(vec2(centerX - ui.measureText(warn2).x/2, boxY + 120))
                    ui.textColored(warn2, rgbm(1, 0.5, 0, 1))
                end
            end

            -- Отрисовка локального штрафа
            if activePenaltySeconds > 0 then
                local cx = ui.windowSize().x / 2
                local cy = ui.windowSize().y / 2
                ui.pushFont(ui.Font.Title)
                local warnText = "PENALTY: STOP YOUR CAR!"
                ui.setCursor(vec2(cx - ui.measureText(warnText).x/2, cy - 100))
                ui.textColored(warnText, rgbm(1, 0, 0, 1))
                
                local timeText = string.format("%.1f", activePenaltySeconds) .. "s"
                ui.setCursor(vec2(cx - ui.measureText(timeText).x/2, cy - 60))
                ui.textColored(timeText, rgbm(1, 1, 0, 1))
                ui.popFont()
            end
        end
    end)
