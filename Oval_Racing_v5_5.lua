-- DRS World: Oval Racing System v5.5 (STRUCT FIX)
-- Fixed state_sim member error | Universal session detection

local VERSION = "OVAL v5.5"
...
function script.update(dt)
    ...
    local ok, err = pcall(function()
        ...
        -- FIX: Универсальный способ получения индекса сессии
        local currentSess = s.session or 0
        if currentSess ~= lastSessionIndex then
