-- DRS World: Oval Racing System v5.2 (SC MOVEMENT FIX)
-- Fixed virtual SC speed | Dynamic track length | Debug info

local VERSION = "OVAL v5.2"
...
local scPos = 0 
local scSpeed = 120 / 3.6 -- 120 км/ч в м/с
...
function script.update(dt)
    ...
    pcall(function()
        ...
        trackLength = math.max(100, s.trackLengthM) -- Обновляем длину трассы
            ...
            for i = 0, ac.getSim().carsCount - 1 do
                ...
                if car and car.isConnected and not car.isInPitlane and car.speedKmh < 40 then
                    ...
                    if slowTimers[i] > 2.0 then 
                        ac.sendChatMessage("CAUTION! (Auto Trigger)")
        ...
        -- Движение Safety Car и проверка обгонов
        if currentFlag ~= STATE.GREEN then
            -- ДВИГАЕМ ВИРТУАЛЬНУЮ ТОЧКУ
            ...
            -- Штраф за обгон под желтыми (дистанция меньше -3 метров)
...
function script.drawUI()
    ...
    ui.textColored(VERSION .. " | SC_Pos: " .. string.format("%.3f", scPos) .. " | Trk: " .. math.floor(trackLength) .. "m", rgbm(1, 1, 1, 0.7))
