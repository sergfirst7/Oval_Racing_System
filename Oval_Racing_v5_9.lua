...
local function applyRealPenalty(id, seconds)
    -- Sending the standard Emperor Server penalty command
    ac.sendChatMessage("/add_time " .. id .. " " .. seconds)
...
function script.update(dt)
    ...
    local ok, err = pcall(function()
        ...
        pcall(function()
            ...
            if sess then
                isRace = (sess.type == ac.SessionType.Race or sess.type == 2 or (sess.name and sess.name:lower():find("race") ~= nil))
            end
        end)
    ...
    pcall(function()
        ...
        if showBlock then
            ...
            if currentFlag ~= STATE.GREEN then
                ...
                if warningLagTimer > 0 and warningLagTimer < 10.0 then
